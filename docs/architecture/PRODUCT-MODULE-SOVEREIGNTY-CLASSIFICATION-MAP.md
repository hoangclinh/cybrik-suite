...
* **GOVERNANCE_OR_DOCUMENTATION**: Governance policies, ADRs, sprint closure dossiers, runbooks, security disclosures, license dossiers, typing markers.
...
| `src/control-plane/cybrik_fabric_control/__init__.py` | `GOVERNANCE_OR_DOCUMENTATION` | `SCAFFOLD` | Control plane package namespace scaffold (implements no product concern at RC1). |
...
| `deploy/ (docker/docker-compose*.yml, pf/docker-compose*.yml, pf/staging/docker-compose.staging.yml, cron/, systemd/, log-collection/agent/, log-collection/fluent-bit/, log-collection/vector/, pf/certs/, pf/opensearch/, pf/*.sh, docker/initdb/, docker/dev-up.sh, pf/.env.t1.example)` | `DEPLOYMENT_PROFILE_OR_CONFIG` | `IMPLEMENTED` | Executable Docker Compose topologies, systemd services/timers, cron jobs, shell launch scripts, environment configuration templates (.env.t1.example), and log agent configurations. |
...
| `services/api/src/cybrik_soc/modules/copilot/ (shadow_remote_contract.py, __init__.py)` | `PRODUCT_CORE` | `IMPLEMENTED` | Copilot shadow remote contracts and module namespace entrypoint. |
...
| `services/api/src/cybrik_soc/platform/svc_delegation/ (algorithms.py, errors.py, models.py, scopes.py, __init__.py)` | `PRODUCT_CORE` | `IMPLEMENTED` | Service delegation token model, error types, algorithms, and RFC 9068 minting specifications. |

...
| `services/api/src/cybrik_soc/modules/ingest/ (source_labels.py, time_guard.py)` | `PRODUCT_CORE` | `IMPLEMENTED` | Ingestion source labels, time verification, and core domain invariants. |
| `services/api/src/cybrik_soc/modules/ingest/ (api.py, ecs.py, field_maps.py, log_parsers.py, log_parsers_bsd.py, log_parsers_ext.py, models.py, normalizers.py, ocsf.py, pf_bridge.py, security_onion.py, service.py, source_health.py, source_health_worker.py)` | `PRODUCT_IMPLEMENTATION_ADAPTER` | `IMPLEMENTED` | FastAPI endpoints, log ingestion wire format parsers (ECS, OCSF, CEF, LEEF, RFC 3164/5424, Windows Event XML, Cisco, FortiGate), field mappers, normalizers, SQLAlchemy models, packet-fabric consumer bridge, Security Onion adapter, and background health polling worker. |
...
| `services/api/src/cybrik_soc/platform/svc_delegation/ (errors.py, models.py, scopes.py)` | `PRODUCT_CORE` | `IMPLEMENTED` | Service delegation token domain model, error types, and RFC 9068 minting specifications. |
| `services/api/src/cybrik_soc/platform/svc_delegation/ (algorithms.py, factory.py, issuer.py, principal_adapter.py, signer.py)` | `PRODUCT_IMPLEMENTATION_ADAPTER` | `IMPLEMENTED` | Algorithm registry adapter (PyJWT algorithm registry binding), delegation token issuer, signer adapter, factory, and authentication principal adapter. |

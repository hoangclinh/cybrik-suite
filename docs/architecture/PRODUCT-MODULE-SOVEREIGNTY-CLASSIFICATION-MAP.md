...
When a historical RC1 source file combines an abstract port/contract interface with an embedded test/demo double adapter (e.g. `services/api/src/cybrik_soc/modules/soar/connectors/__init__.py`), it is classified at the file level by its concrete executable realization (`PRODUCT_IMPLEMENTATION_ADAPTER`). Traceability notes explicitly document the mixed composition to avoid overstating runtime readiness or attributing production execution authority to the test double.

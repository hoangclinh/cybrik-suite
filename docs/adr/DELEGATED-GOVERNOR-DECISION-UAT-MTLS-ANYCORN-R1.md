...
1. require the exact `SUITE_COMMIT`, `SUITE_TREE`, canonical clean detached `SUITE_ROOT` and
   ...
   is a hard stop;
2. create with `mkdir`, never `mkdir -p`, one fresh mode-`0700` outside-repository tool root whose
   ...
   mode `0700`; record each root's `st_dev` and `st_ino`. The evidence root may not be under `/tmp`
   or `/private/tmp`. Neither root may contain, be contained by, or be an ancestor/descendant of a
...
3. permit writes inside the tool root only to the exact subpaths `<COVERAGE_ROOT>/wheel`,
...
4. fetch only
   ...
   A non-empty OSV vulnerability result is a hard stop before extraction; write the bounded raw
...
5. require wheel size `221943` and SHA-256
...
6. use only the absolute `PINNED_PYTHON=` path and `PINNED_PYTHON_REALPATH=` recorded in the
...
7. extract the already-verified wheel with exactly `<PINNED_PYTHON> -m zipfile -e
...
8. record the authorization fields, root identities, wheel size/digest, exact OSV request and raw
   ...
   integrity checks define `D2-COV-P0-INSTALL=PASS`; the coverage percentage remains a separate
   `HOLD` gate;
9. on any failure or rollback, write one bounded secret-free failure record, remove only the

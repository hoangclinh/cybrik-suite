...
- **Status:** `PROPOSED — READY FOR CANONICAL MERGE`

---

## 1. Executive Context & Invariants

This matrix establishes the authoritative, cross-product reconciliation for the **10 Canonical Capability Pillars** defined in [`docs/strategy/06-ROADMAP-2026-2029.md`](file:///Users/hoanglinh/Claude/Projects/soc-autonomous-state/worktrees/wt-open-contracts-consolidation/docs/strategy/06-ROADMAP-2026-2029.md). 
...
|---|---|---|---|---|---|---|
| **1** | **Local LLM Runtime & Multi-Model Routing** | `cybrik.ai-bom.v1.schema.json`<br>`cybrik.model-inference-request.v1.schema.json`<br>`cybrik.model-capability.v1.schema.json` | `cybrik-cyber-ai-platform`<br>`cybrik-suite` | **IMPLEMENTED_UNINTEGRATED (COHORT_2026_01_IN_REMEDIATION)**<br>(T0/T1/T2 Runtimes: vLLM, Ollama, llama.cpp, Triton, Stub) | `test_stub_adapter.py`<br>`test_ollama_adapter.py`<br>`test_competitive_schemas.py` | **Superior to MS Copilot / Charlotte AI** on air-gap sovereignty, no-cloud-egress, zero training data leakage, and exact AI-BOM model provenance. |
| **2** | **Policy-Enforced RAG & Data Ingestion** | `cybrik.ai-bom.v1.schema.json`<br>`cybrik.data-marking.v1.schema.json`<br>`cybrik.org-scope-grant.v1.schema.json` | `cybrik-cyber-ai-platform`<br>`cybrik-soc-command-center` | **IMPLEMENTED_UNINTEGRATED (COHORT_2026_01_IN_REMEDIATION)**<br>(PostgreSQL + pgvector, quarantine, classification filter) | `test_marking.py`<br>`test_egress.py`<br>`validate-runtime-admission.test.mjs` | **Superior to Cortex XSIAM & Splunk** on strict pre-ranking tenant isolation, TLP enforcement, and signed offline knowledge pack updates. |
| **3** | **Threat Intelligence & Structured CTI** | `cybrik.stix-cti-bundle.v1.schema.json`<br>`cybrik.data-marking.v1.schema.json` | `cybrik-cyber-ai-platform`<br>`cybrik-soc-command-center` | **IMPLEMENTED_UNINTEGRATED (COHORT_2026_01_IN_REMEDIATION)**<br>(STIX 2.1, ATT&CK, CVE, CISA KEV, TAXII 2.1) | `test_competitive_schemas.py`<br>`cybrik.stix-cti-bundle.v1.schema.json`<br>MISP/OpenCTI offline importer | **Matches/Exceeds Sentinel / OpenCTI** with native CISA KEV ransomware tracking, ATT&CK technique pinning, and cryptographic citation linking. |
| **4** | **Bounded Durable Multi-Agent Orchestrator** | `cybrik.investigation-bundle.v1.schema.json`<br>`cybrik.investigation-checkpoint.v1.schema.json`<br>`cybrik.investigation-create-request.v1.schema.json` | `cybrik-cyber-ai-platform`<br>`cybrik-soc-command-center` | **IMPLEMENTED_UNINTEGRATED (COHORT_2026_01_IN_REMEDIATION)**<br>(Durable state machine, checkpoints, replay, contradiction detection) | `test_orchestration_controller.py`<br>`test_orchestration_checkpoints.py`<br>`test_investigation_service.py` | **Superior to LangChain / AutoGen** with deterministic checkpoint recovery, step/token resource bounds, explicit abstention, and graph bundles. |
| **5** | **Signed Capability Registry & MCP Gateway** | `cybrik.capability.v1.schema.json`<br>`cybrik.tool-execution-request.v1.schema.json`<br>`cybrik.svc-delegation-token.v1.schema.json` | `cybrik-security-tool-fabric`<br>`cybrik-suite` | **IMPLEMENTED_UNINTEGRATED (COHORT_2026_01_IN_REMEDIATION)**<br>(REST / MCP 2025-11-25 gateway, signed manifest, kill switches) | `test_capability_conformance.py`<br>`test_wire_contracts.py`<br>`test_svc_delegation.py` | **Matches Torq / Cortex XSOAR** on protocol extensibility while providing SPIFFE identity delegation and cryptographic kill switches. |
| **6** | **Security Investigation Tool Packs** | `cybrik.soc-get-alert-context.v1.schema.json`<br>`cybrik.tool-execution-result.v1.schema.json` | `cybrik-security-tool-fabric`<br>`cybrik-soc-command-center` | **IMPLEMENTED_UNINTEGRATED (COHORT_2026_01_IN_REMEDIATION)**<br>(R0–R3 typed packs: SIEM, OpenSearch, Wazuh, IOC, CVE, PCAP) | `test_alert_context_conformance.py`<br>`validate-alert-context-transport.test.mjs` | **Matches Splunk ES / Sentinel** with zero-trust scoped query bindings, strict redaction, and no unrestricted stdout leakage. |
| **7** | **Isolated Analysis Sandbox** | `cybrik.execution-receipt.v1.schema.json`<br>`cybrik.common-defs.v1.schema.json` ($defs/isolationProfile) | `cybrik-security-tool-fabric` | **IMPLEMENTED_UNINTEGRATED (COHORT_2026_01_IN_REMEDIATION)**<br>(S0 pooled, S1/S2 disposable container, network-isolated) | `test_fabric_partition.py`<br>`test_validate_evidence_chain.py` | **Matches Cuckoo / Joe Sandbox** with lightweight ephemeral S1/S2 execution, archive bomb guards, and derived artifact provenance hashes. |
| **8** | **Detection Engineering Lifecycle** | `cybrik.capability.v1.schema.json`<br>`cybrik.investigation-bundle.v1.schema.json` | `cybrik-security-tool-fabric`<br>`cybrik-soc-command-center`<br>(Fabric control plane & SOC detection bridge) | **IMPLEMENTED_UNINTEGRATED (COHORT_2026_01_IN_REMEDIATION)**<br>(Sigma, YARA, YARA-X, Suricata compile/replay/test/rollback) | `test_scaffold_integrity.py`<br>`validate-platform-contract.test.mjs` | **Matches SOC Prime / Detection-as-Code** with automated false-positive backtesting, syntax compilation, and signed rule rollback. |
| **9** | **Controlled Action & Approval Broker** | `cybrik.approval-request.v1.schema.json`<br>`cybrik.approval-decision.v1.schema.json`<br>`cybrik.execution-receipt.v1.schema.json` | `cybrik-security-tool-fabric`<br>`cybrik-soc-command-center` | **IMPLEMENTED_UNINTEGRATED (COHORT_2026_01_IN_REMEDIATION)**<br>(R0–R3 risk tiers, four-eyes gate, credential/egress broker, A4) | `test_policy.py`<br>`test_egress.py`<br>`validate-receipt-integrity.test.mjs` | **Superior to SOAR platforms (Torq, Swimlane, XSOAR)** by eliminating long-lived credentials, preventing confused-deputy attacks, and enforcing fail-closed TTLs. |
| **10** | **Immutable Audit, AI-BOM & Evaluation** | `cybrik.execution-receipt-ledger.v1.schema.json`<br>`cybrik.ai-bom.v1.schema.json`<br>`cybrik.receipt-trust-bundle.v1.schema.json` | `cybrik-suite`<br>`cybrik-security-tool-fabric`<br>`cybrik-cyber-ai-platform` | **IMPLEMENTED_UNINTEGRATED (COHORT_2026_01_IN_REMEDIATION)**<br>(Merkle hash ledger, AI-BOM, golden/adversarial eval suites) | `test_competitive_schemas.py`<br>`test_receipt_integrity.py`<br>`validate-receipt-trust-durability.test.mjs` | **Industry-First Innovation** exceeding all commercial competitors in end-to-end cryptographic replay, Merkle ledger auditing, and AI supply-chain provenance. |
...
|---|---|---|---|---|
| **1** | **CYBRIK Native** | Alert, Case, Asset, IOC, Audit, Data Plane | **VERIFIED** | `test_soc_ai_lifecycle_create.py`, `validate-alert-context.test.mjs` |
| **2** | **Event / SIEM** | OpenSearch, Wazuh, Syslog, CEF, OCSF | **VERIFIED** | Conformance fixtures in `wt-impl-fabric-consolidated` |
| **3** | **Network Security** | Suricata, Security Onion, PCAP Replay | **ACTIVE** | S2 sandbox replay harness, Suricata syntax compiler |
| **4** | **CTI** | STIX 2.1, TAXII 2.1, MISP, OpenCTI | **VERIFIED** | `cybrik.stix-cti-bundle.v1.schema.json`, `test_competitive_schemas.py` |
| **5** | **Vulnerability & Knowledge** | CVE, CISA KEV, MITRE ATT&CK v15+ | **VERIFIED** | STIX 2.1 vulnerability extension, offline knowledge packs |
| **6** | **Endpoint / Artifact** | Generic Artifact Intake, Host Isolation Adapter | **ACTIVE** | S1 sandbox extractor, SHA-256 artifact manifest |
| **7** | **Response & Remediation** | OpenC2, Typed Webhook, Reversible Block | **ACTIVE** | R3 approval broker, credential broker, TTL rollback |
| **8** | **Extensibility & SDK** | REST, MCP 2025-11-25, Connector SDK | **VERIFIED** | `cybrik.capability.v1.schema.json`, conformance test kit |
...
```mermaid
flowchart TD
    subgraph Core_Contracts ["Phase 1: Canonical Contract Baseline (Wave 0-2 Closure)"]

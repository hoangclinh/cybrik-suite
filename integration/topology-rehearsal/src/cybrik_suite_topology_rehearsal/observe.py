...
# The mapping-view machinery this module was authored around and still reads through. It was
# moved out whole so this file stays inside its reviewed size bound; nothing about it changed.
# `IMMUTABLE_LEAVES` and `immutability_findings` are not called here any more — they are read
# from this module by `preparation`, so they are imported rather than reached for across it,
# and every name a caller already reached for through `observe` still resolves through
# `observe`. They stay out of the curated `__all__` below for the same reason
# `platform_evidence` does: they are package plumbing, not a reducer a caller reaches for.
from .views import (
    IMMUTABLE_LEAVES,

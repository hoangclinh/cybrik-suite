...
2. Cryptographic signature and trust anchoring via detached Ed25519 signatures ONLY (`manifest.sig`, strictly 64 octets binary) over RFC 8785 JSON Canonicalization Scheme (JCS) canonical `manifest.json` under strict I-JSON (RFC 7493) constraints, proposed under OPEN-1 to supersede parent Platform Contract Proposal §7's embedded signature model while preserving parent proposal byte-stability pending future Founder decision.
...
This specification formally elaborates **OPEN-1 (`OFFLINE_INSTALL_UPDATE_CONTRACT`)** subordinate to the parent Platform Contract Proposal (`CYBRIK-PLATFORM-CONTRACT-V1-PROPOSAL.md`). Parent Proposal §7 originally described a manifest signing scheme where a `bundle_signature` field was embedded within the manifest and omitted during canonicalization. Under this OPEN-1 specification, the design is elaborated and unified on the pure detached signature model (`manifest.sig` companion file + `detached_signature` metadata declaration object), which is proposed to supersede parent §7's embedded signature formulation by ensuring bit-for-bit canonical serialization without in-document mutation or field-stripping semantics. In accordance with architecture governance and auditability invariants, the parent `CYBRIK-PLATFORM-CONTRACT-V1-PROPOSAL.md` document maintains strict byte-stability pending future Founder acceptance of this OPEN-1 contract specification.
...
The archive root MUST contain exactly one canonical update manifest named `manifest.json` and its companion detached cryptographic signature named `manifest.sig` at the top level, implementing the OPEN-1 detached signature model proposed to supersede parent Platform Contract Proposal §7's embedded signature formulation (preserving parent proposal byte-stability pending future Founder decision). All referenced artifact files MUST reside within subdirectories relative to the archive root (root `manifest.json` and `manifest.sig` are envelope control files and are strictly prohibited from appearing in `artifacts[]`).
...
- **Embedded / In-Band Header Signatures Rejected:** In accordance with the OPEN-1 detached signature specification (which proposes to supersede parent Platform Contract Proposal §7's embedded signature formulation while preserving parent document byte-stability pending future Founder decision), in-band signature envelopes (e.g., JOSE/JWT headers with embedded JWK/x5c key declarations) and embedded signature fields inside manifest JSON MUST NOT be used for bundle authentication.
...
4. **Detached Envelope Verification & Parent Contract §7 Elaboration:** In accordance with the detached signature model elaborated under OPEN-1 (which proposes to supersede parent Platform Contract Proposal §7's embedded signature model while preserving parent document byte-stability pending future Founder acceptance), the control files `manifest.json` and `manifest.sig` form a detached verification envelope. The update station validates `manifest.sig` directly against `manifest.json` before unpacking or processing archive payload artifacts. Because `manifest.json` and `manifest.sig` are envelope control files evaluated prior to payload processing, they MUST NOT be listed in `artifacts[]` (preventing circular hashing dependencies and ensuring unambiguous separation between trust metadata and payload artifacts).

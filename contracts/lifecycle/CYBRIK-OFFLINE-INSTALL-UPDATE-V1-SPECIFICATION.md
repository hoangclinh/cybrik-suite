...
2. Cryptographic signature and trust anchoring via detached Ed25519 signatures ONLY (`manifest.sig`, strictly 64 octets binary) over RFC 8785 JSON Canonicalization Scheme (JCS) canonical `manifest.json` under strict I-JSON (RFC 7493) constraints, directly aligned with parent Platform Contract Proposal §7.
...
- **Embedded / In-Band Header Signatures Rejected:** In accordance with parent Platform Contract Proposal §7, in-band signature envelopes (e.g., JOSE/JWT headers with embedded JWK/x5c key declarations or embedded signature fields) MUST NOT be used for bundle authentication.
...
This specification fully elaborates and satisfies the architectural and contract requirements for **OPEN-1 (`OFFLINE_INSTALL_UPDATE_CONTRACT`)** subordinate to parent Platform Contract Proposal §7 and ADR-0015 §14.

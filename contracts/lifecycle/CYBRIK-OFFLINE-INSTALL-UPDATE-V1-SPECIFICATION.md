...
| `HEALTH_PROBE` | Execute deterministic HTTP GET/HEAD readiness/liveness probe | Local loopback / private enclave URI matching `^https?:\/\/(?:localhost\|127\.0\.0\.1\|\[::1\]\|10\.(?:25[0-5]\|2[0-4][0-9]\|1[0-9]{2}\|[1-9]?[0-9])\.(?:25[0-5]\|2[0-4][0-9]\|1[0-9]{2}\|[1-9]?[0-9])\.(?:25[0-5]\|2[0-4][0-9]\|1[0-9]{2}\|[1-9]?[0-9])\|192\.168\.(?:25[0-5]\|2[0-4][0-9]\|1[0-9]{2}\|[1-9]?[0-9])\.(?:25[0-5]\|2[0-4][0-9]\|1[0-9]{2}\|[1-9]?[0-9])\|172\.(?:1[6-9]\|2[0-9]\|3[0-1])\.(?:25[0-5]\|2[0-4][0-9]\|1[0-9]{2}\|[1-9]?[0-9])\.(?:25[0-5]\|2[0-4][0-9]\|1[0-9]{2}\|[1-9]?[0-9]))(?::(?:[1-9][0-9]{0,3}\|[1-5][0-9]{4}\|6[0-4][0-9]{3}\|65[0-4][0-9]{2}\|655[0-2][0-9]\|6553[0-5]))?(?:\/.*)?$` | Optional `{"method": "GET"\|"HEAD", "expected_status": int(200-599), "interval_seconds": int(>=1), "retries": int(>=0)}` | Read-only idempotent; zero outbound WAN network egress; timeout bounded; failure triggers rollback |
| `RESTORE_DATABASE_SNAPSHOT` | Restore database schema/data from verified pre-apply snapshot | Relative snapshot path matching `^(?!/)(?!^\./)(?!.*\.\.)(?!.*(?:/\.|//|/$))[a-z0-9._/-]+\.(?:sql|db|bak)$` | `{}` (empty object) | Idempotent restoration on rollback |

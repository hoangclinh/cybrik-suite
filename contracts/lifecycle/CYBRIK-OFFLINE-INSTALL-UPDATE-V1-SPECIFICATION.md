...
- **Snapshot Binding & Safe Restoration Parameter Closure:** For `RESTORE_DATABASE_SNAPSHOT` actions, `target` is strictly bound to relative snapshot paths (`snapshots/<path>.(sql|db|bak)`) or pre-apply snapshot variables (`$PRE_APPLY_SNAPSHOT/<path>.(sql|db|bak)`). Snapshot restore target paths MUST adhere to strict path construction constraints: no empty path segments or double slashes (`//`), no leading dots on any path segment (`/.`), no parent directory traversal sequences (`..`), no trailing slashes (`/`), and no absolute host filesystem paths (e.g., `/etc/passwd.db` or root paths). Any violating target path is unconditionally rejected to eliminate arbitrary file overwrite and host filesystem escape vectors.
...
| `RESTORE_DATABASE_SNAPSHOT` | Restore database schema/data from verified pre-apply snapshot | Relative snapshot journal path or pre-apply snapshot variable matching `^(?:snapshots\/|\$PRE_APPLY_SNAPSHOT\/)(?!.*\.\.)(?!.*(?:\/\.|\/\/|\/$))[a-zA-Z0-9._/-]+\.(?:sql|db|bak)$` | `{}` (empty object) | Idempotent restoration on rollback; strictly bound to snapshot journal or pre-apply snapshot variable; absolute host paths rejected |

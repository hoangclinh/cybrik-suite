# CYBRIK Offline Installation & Update Manifest Contract Specification (v0.1.0)
...
2. Cryptographic signature and trust anchoring via detached Ed25519 signatures ONLY (`manifest.sig`, strictly 64 octets binary) over RFC 8785 JSON Canonicalization Scheme (JCS) canonical `manifest.json` under strict I-JSON (RFC 7493) constraints, accepted under OPEN-1 to supersede parent Platform Contract Proposal §7's embedded signature model while preserving parent proposal byte-stability under Founder acceptance on 2026-08-29.
...
#### Relationship to Parent Platform Contract Proposal (§7)
This specification formally elaborates **OPEN-1 (`OFFLINE_INSTALL_UPDATE_CONTRACT`)** subordinate to the parent Platform Contract Proposal (`CYBRIK-PLATFORM-CONTRACT-V1-PROPOSAL.md`). Parent Proposal §7 originally described a manifest signing scheme where a `bundle_signature` field was embedded within the manifest and omitted during canonicalization. Under this OPEN-1 specification, the design is elaborated and unified on the pure detached signature model (`manifest.sig` companion file + `detached_signature` metadata declaration object), which supersedes parent §7's embedded signature formulation by ensuring bit-for-bit canonical serialization without in-document mutation or field-stripping semantics. In accordance with architecture governance and auditability invariants, the parent `CYBRIK-PLATFORM-CONTRACT-V1-PROPOSAL.md` document maintains strict byte-stability under Founder acceptance of this OPEN-1 contract specification on 2026-08-29.
...
The archive root MUST contain exactly one canonical update manifest named `manifest.json` and its companion detached cryptographic signature named `manifest.sig` at the top level, implementing the OPEN-1 detached signature model accepted to supersede parent Platform Contract Proposal §7's embedded signature formulation (preserving parent proposal byte-stability under Founder acceptance on 2026-08-29). All referenced artifact files MUST reside within subdirectories relative to the archive root (root `manifest.json` and `manifest.sig` are envelope control files and are strictly prohibited from appearing in `artifacts[]`).
...
4. **Detached Ed25519 Signature Model (Superseding Parent §7 Embedded Signature):** Parent Platform Contract Proposal §7 originally specified that `bundle_signature` be computed over canonical JSON omitting the signature field itself. OPEN-1 elaborates this requirement and adopts the pure detached signature model (`manifest.sig` + `detached_signature` declaration object) to supersede parent §7's embedded signature approach. Under this model, the cryptographic signature MUST be provided as a detached file `manifest.sig` (located in the pax archive root or delivered alongside `manifest.json`) containing the exact 64-byte Ed25519 binary signature over the RFC 8785 JCS canonical UTF-8 byte stream of `manifest.json`. Embedded signatures, in-band signature envelopes, or signature mirroring inside `manifest.json` are strictly prohibited to ensure deterministic bit-for-bit manifest payload hashing without requiring runtime omission rules or mutating manifest bytes. The manifest JSON includes a `detached_signature` declaration object specifying the algorithm (`"ed25519"`), signature file (`"manifest.sig"`), and the SHA-256 SPKI fingerprint of the signing key (`key_fingerprint`). The `detached_signature.key_fingerprint` MUST exactly match `operator_trust_root.public_key_fingerprint`. The parent contract proposal text (`CYBRIK-PLATFORM-CONTRACT-V1-PROPOSAL.md`) remains byte-stable under Founder acceptance on 2026-08-29.
...
- **Embedded / In-Band Header Signatures Rejected:** In accordance with the OPEN-1 detached signature specification (which supersedes parent Platform Contract Proposal §7's embedded signature formulation while preserving parent document byte-stability under Founder acceptance on 2026-08-29), in-band signature envelopes (e.g., JOSE/JWT headers with embedded JWK/x5c key declarations) and embedded signature fields inside manifest JSON MUST NOT be used for bundle authentication.
...
- **Snapshot Binding & Safe Restoration Parameter Closure:** For `RESTORE_DATABASE_SNAPSHOT` actions, `target` is strictly bound to relative snapshot paths (`snapshots/<path>.(sql|db|bak)`) or pre-apply snapshot variables (`$PRE_APPLY_SNAPSHOT/<path>.(sql|db|bak)`) conforming to the exact hardened regex `^(?:snapshots|\$PRE_APPLY_SNAPSHOT)\/(?!.*\/\/)((?!\.)[a-zA-Z0-9_\-\.]+\/)*(?!\.)[a-zA-Z0-9_\-\.]+\.(?:sql|db|bak)$`. Snapshot restore target paths MUST adhere to strict path construction constraints: no empty path segments or double slashes (`//`), no leading dots on any path segment (`/.`), no parent directory traversal sequences (`..`), no trailing slashes (`/`), and no absolute host filesystem paths (e.g., `/etc/passwd.db` or root paths). Paths containing leading dot segments such as `snapshots/.hidden.db` or `$PRE_APPLY_SNAPSHOT/.hidden.db`, double slashes such as `snapshots//backup.db`, or invalid extensions MUST be rejected unconditionally to eliminate arbitrary file overwrite and host filesystem escape vectors.
...
4. **Detached Envelope Verification & Parent Contract §7 Elaboration:** In accordance with the detached signature model elaborated under OPEN-1 (which supersedes parent Platform Contract Proposal §7's embedded signature model while preserving parent document byte-stability under Founder acceptance on 2026-08-29), the control files `manifest.json` and `manifest.sig` form a detached verification envelope. The update station validates `manifest.sig` directly against `manifest.json` before unpacking or processing archive payload artifacts. Because `manifest.json` and `manifest.sig` are envelope control files evaluated prior to payload processing, they MUST NOT be listed in `artifacts[]` (preventing circular hashing dependencies and ensuring unambiguous separation between trust metadata and payload artifacts).
...
| `RESTORE_DATABASE_SNAPSHOT` | Restore database schema/data from verified pre-apply snapshot | Relative snapshot journal path or pre-apply snapshot variable matching `^(?:snapshots|\$PRE_APPLY_SNAPSHOT)\/(?!.*\/\/)((?!\.)[a-zA-Z0-9_\-\.]+\/)*(?!\.)[a-zA-Z0-9_\-\.]+\.(?:sql|db|bak)$` | `{}` (empty object) | Idempotent restoration on rollback; strictly bound to snapshot journal or pre-apply snapshot variable; absolute host paths, hidden dot segments (`snapshots/.hidden.db`, `$PRE_APPLY_SNAPSHOT/.hidden.db`), and double slashes (`snapshots//backup.db`) rejected |
...
This specification fully elaborates and satisfies the architectural and contract requirements for **OPEN-1 (`OFFLINE_INSTALL_UPDATE_CONTRACT`)** subordinate to parent Platform Contract Proposal §7 and ADR-0015 §14, with the detached signature model accepted to supersede parent §7's embedded signature formulation while preserving parent proposal byte-stability under Founder acceptance on 2026-08-29.

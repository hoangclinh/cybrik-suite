...
| `HEALTH_PROBE` | Execute deterministic HTTP GET or TCP readiness/liveness probe | Local loopback URI matching `^https?://(?:127\.0\.0\.1|localhost|\[::1\])(?::[0-9]{1,5})?(?:/[a-zA-Z0-9._~%!$&'()*+,;=:@/-]*)?$` | Optional `{"method": "GET"\|"HEAD", "expected_status": int(200-599), "interval_seconds": int(>=1), "retries": int(>=0)}` | Timeout bounded; failure triggers rollback; WAN egress and POST prohibited |

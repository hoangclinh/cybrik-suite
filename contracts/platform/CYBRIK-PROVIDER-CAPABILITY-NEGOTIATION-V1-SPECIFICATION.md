...
2. **Evidence Binding & URN Naming**: Every claimed capability MUST reference verifiable conformance test evidence (`conformance_evidence`) formatted as structured URNs matching `^urn:cybrik:evidence:[a-zA-Z0-9_-]+(?::[a-zA-Z0-9_-]+)*$`. For storage Object Lock capabilities, evidence references MUST follow canonical Object Lock evidence URN naming (canonical pattern `urn:cybrik:evidence:storage:object-lock:v1` or namespace-qualified variant `urn:cybrik:evidence:storage-object-lock:...`) and require an exact 1-to-1 matching with a `conformance_evidence[].test_identifier` entry verifying test execution with `status: "PASS"`. Unbacked claims or dangling references MUST be treated as absent and fail validation.
...
* `advertised_capabilities`: Array of capability descriptors declaring `slot_id`, `is_mandatory`, `supported_features`, `degradation_fallback` details, and structured `evidence_references` URNs matching `^urn:cybrik:evidence:[a-zA-Z0-9_-]+(?::[a-zA-Z0-9_-]+)*$`.
* `conformance_evidence`: Concrete test execution records (`test_identifier` matching URN pattern `^urn:cybrik:evidence:[a-zA-Z0-9_-]+(?::[a-zA-Z0-9_-]+)*$`, required `status: "PASS"`, required 64-character hex `evidence_pack_digest` matching `^[a-f0-9]{64}$`, and optional `executed_at` timestamp / `report_uri`). Each record verifies test execution with an exact 1-to-1 matching requirement against capability evidence references, confirmed passing status (`status: "PASS"`), and a 64-character hex `evidence_pack_digest`.
...
4. Issues the signed/authenticated `agreed_capability_lease` containing active runtime configuration. The agreed capability lease (`negotiated_optional_capabilities`) MUST contain exactly the same multiset of `(capability_name, slot_id)` pairs as requested in `negotiation_request.requested_optional_capabilities`—no surplus, omitted, or unrequested entries permitted.
...
| `/advertisement_response/advertised_capabilities[].evidence_references` | `array` | MinItems 1, items pattern `^urn:cybrik:evidence:[a-zA-Z0-9_-]+(?::[a-zA-Z0-9_-]+)*$` | Evidence URN references backing the advertised capability. For storage Object Lock, canonical naming is `urn:cybrik:evidence:storage:object-lock:v1` or `urn:cybrik:evidence:storage-object-lock:...`. |
| `/advertisement_response/conformance_evidence[].test_identifier` | `string` | Pattern `^urn:cybrik:evidence:[a-zA-Z0-9_-]+(?::[a-zA-Z0-9_-]+)*$` | Unique test evidence URN identifier (e.g. `urn:cybrik:evidence:storage:object-lock:v1` or `urn:cybrik:evidence:storage-object-lock:v1`), requiring exact 1-to-1 matching with `evidence_references` and verified passing status (`status: "PASS"`). |
| `/advertisement_response/conformance_evidence[].status` | `string` | Enum `["PASS", "FAIL", "INCONCLUSIVE", "SKIPPED"]` | Test execution status; every valid conformance evidence item MUST carry `status: "PASS"`. |
| `/advertisement_response/conformance_evidence[].evidence_pack_digest` | `string` | Pattern `^[a-f0-9]{64}$` | 64-character lowercase hexadecimal SHA-256 digest of the raw evidence pack artifact. |
...
| `/agreed_capability_lease/negotiated_optional_capabilities` | `array` | Unique composite keys `(capability_name, slot_id)` | Array of resolved optional capability descriptors. `negotiation_request.requested_optional_capabilities` and `agreed_capability_lease.negotiated_optional_capabilities` MUST satisfy strict bidirectional composite-identity `(capability_name, slot_id)` multiset equality; any surplus, leftover, or unrequested lease entry is strictly prohibited. |
...
1. **Evidence Referential Integrity (`SR-NEG-01`)**: Every item in `evidence_references` within `advertisement_response.advertised_capabilities` MUST be a valid URN matching `^urn:cybrik:evidence:[a-zA-Z0-9_-]+(?::[a-zA-Z0-9_-]+)*$` and MUST resolve to an exact 1-to-1 `test_identifier` defined in `advertisement_response.conformance_evidence` verifying passing execution (`status: "PASS"`) and carrying a 64-character hex `evidence_pack_digest`.

...
| `/advertisement_response/claim_type` | `string` | Enum `["PARTIAL_CAPABILITY_ADVERTISEMENT", "FULL_PROFILE_CONFORMANCE_DECLARATION"]` | Scope of the capability claim. Full-profile conformance declarations (`FULL_PROFILE_CONFORMANCE_DECLARATION`) strictly declare all 13 capability slots regardless of negotiation outcome or disposition (even for `REJECTED_FAIL_CLOSED`), with `is_mandatory`, `supported_features`, `degradation_fallback`, and designate canonical storage evidence references (such as `urn:cybrik:evidence:storage:s3:conformance:v1:s3-17-ops` and the exclusive allowlisted canonical Object Lock evidence `urn:cybrik:evidence:storage:s3:conformance:v1:object-lock`) on the storage slot. ANY advertisement declaring Slot 5 (`storage`), whether a full profile conformance declaration or a partial advertisement, MUST declare the complete 17-operation closed baseline and strictly allow only canonical Object Lock evidence URN. |
...
| `/advertisement_response/advertised_capabilities[].supported_features` | `array` | MinItems 1, non-empty strings | List of supported features declared for the capability slot. For ANY advertisement declaring Slot 5 (`storage`), whether a full profile conformance declaration or a partial advertisement, `supported_features` MUST declare the complete 17-operation closed baseline (`PutObject`, `GetObject`, `HeadObject`, `DeleteObject`, `DeleteObjects`, `ListObjectsV2`, `HeadBucket`, `CreateBucket`, `PutObjectRetention`, `GetObjectRetention`, `PutObjectLegalHold`, `GetObjectLegalHold`, `CreateMultipartUpload`, `UploadPart`, `CompleteMultipartUpload`, `AbortMultipartUpload`, `ListParts`). |
...
| `/advertisement_response/conformance_evidence[].test_identifier` | `string` | Pattern `^urn:cybrik:evidence:[a-zA-Z0-9_-]+(?::[a-zA-Z0-9_-]+)*$` | Unique test evidence URN identifier (for storage capabilities in ANY advertisement whether a full profile conformance declaration or a partial advertisement, supporting dual evidence `urn:cybrik:evidence:storage:s3:conformance:v1:s3-17-ops` and exclusive canonical Object Lock evidence `urn:cybrik:evidence:storage:s3:conformance:v1:object-lock`), requiring strict 1-to-1 bijection with `evidence_references` and verified passing status (`status: "PASS"`). All evidence URNs MUST be lowercase canonical strings matching `^urn:cybrik:evidence:[a-z0-9][a-z0-9-_:]*[a-z0-9]$`; case-variant URNs (e.g. `...:Object-Lock`, `...:OBJECT-LOCK`) are strictly forbidden and rejected. |

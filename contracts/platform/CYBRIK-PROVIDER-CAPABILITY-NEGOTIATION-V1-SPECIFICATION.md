...
4. **Strict Status-Pair Coupling**: Handshake outcome status and lease status MUST be strictly coupled: `AGREED_LEASE_GRANTED` pairs ONLY with `ACTIVE_OPTIMAL` (where zero capabilities are degraded and all fallbacks are `"NONE"`); `DEGRADED_LEASE_GRANTED` pairs ONLY with `ACTIVE_DEGRADED` (requiring at least one capability with `GRANTED_DEGRADED` and a non-`"NONE"` fallback); `TERMINAL_REJECTED` pairs ONLY with `REJECTED_FAIL_CLOSED`.
...
5. **Degradation Integrity & Strict Status-Pair Coupling (`SR-NEG-05`)**: Handshake outcome status and lease status MUST strictly couple: `AGREED_LEASE_GRANTED` pairs ONLY with `ACTIVE_OPTIMAL`, where NO agreed capability may have `disposition: "GRANTED_DEGRADED"` / `status: "GRANTED_DEGRADED"` or a non-`"NONE"` fallback applied (`fallback_applied !== "NONE"`). `DEGRADED_LEASE_GRANTED` pairs ONLY with `ACTIVE_DEGRADED`, requiring at least one agreed capability to have `disposition: "GRANTED_DEGRADED"` (or `status: "GRANTED_DEGRADED"`) with `fallback_applied !== "NONE"`. For profiles requiring immutable storage (`profile.slots.storage.specification.immutable_storage_required === true`), Object Lock / WORM degradation is strictly forbidden.

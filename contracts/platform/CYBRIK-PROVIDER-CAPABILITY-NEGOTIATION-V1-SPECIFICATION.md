...
| `/agreed_capability_lease/negotiated_optional_capabilities` | `array` | Unique composite keys `(capability_name, slot_id)` | Array of resolved optional capability descriptors. Every requested optional capability in `negotiation_request.requested_optional_capabilities` MUST be resolved by matching exact composite identity `(capability_name, slot_id)` and cardinality in `negotiated_optional_capabilities`. |

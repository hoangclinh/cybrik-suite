...
4. `storage` (Reconciled with accepted Platform Contract Proposal §5 under `OPEN-2`: 15-operation closed baseline on non-immutable profiles consisting of 8 CRUD/Bucket operations: `PutObject`, `GetObject`, `HeadObject`, `DeleteObject`, `DeleteObjects`, `ListObjectsV2`, `HeadBucket`, `CreateBucket`; 5 Multipart operations: `CreateMultipartUpload`, `UploadPart`, `CompleteMultipartUpload`, `AbortMultipartUpload`, `ListParts`; and 2 Bucket Versioning operations: `PutBucketVersioning`, `GetBucketVersioning`; extending to 19 operations when Object Lock is included by adding `PutObjectRetention`, `GetObjectRetention`, `PutObjectLegalHold`, `GetObjectLegalHold`)
...
### INV-NEG-05: Payload Type Gating Invariant (`OPEN-2` / `OPEN-5`)
Payload inputs across storage operations and cryptographic digest verification MUST be strictly type-gated:
...
2. **Fail-Closed Type Gating**: Any unsupported or invalid type—specifically including `null`, `undefined` (when a payload argument is required or passed), `Date`, `Uint16Array`, `Function`, plain object, number, boolean, array, or other structured non-byte types—MUST fail closed:
   ...
   - In protocol dispatch and request validation pipelines (`dispatchS3PutObject`, `dispatchS3Error`), fail closed immediately with HTTP 400 `InvalidDigest` and reason `MALFORMED_PAYLOAD_TYPE`.
...
| `/advertisement_response/advertised_capabilities[].supported_features` | `array` | MinItems 1, non-empty strings | List of supported features declared for the capability slot. For ANY advertisement declaring Slot 5 (`storage`), whether a full profile conformance declaration or a partial advertisement, `supported_features` MUST declare the closed storage baseline operations: 15 operations on non-immutable profiles (`PutObject`, `GetObject`, `HeadObject`, `DeleteObject`, `DeleteObjects`, `ListObjectsV2`, `HeadBucket`, `CreateBucket`, `CreateMultipartUpload`, `UploadPart`, `CompleteMultipartUpload`, `AbortMultipartUpload`, `ListParts`, `PutBucketVersioning`, `GetBucketVersioning`) or 19 operations when Object Lock is included (adding `PutObjectRetention`, `GetObjectRetention`, `PutObjectLegalHold`, `GetObjectLegalHold`) as a strictly closed set of enumerated baseline operations; any surplus operation (such as `DeleteBucket`) or undeclared operation is strictly forbidden and rejected. |
...
4. **Storage Slot Operation Baseline (15/19 Ops) & Canonical Object Lock Evidence Verification (`SR-NEG-04`)**: ANY advertisement declaring Slot 5 (`storage`), whether a full profile conformance declaration (`FULL_PROFILE_CONFORMANCE_DECLARATION`) or a partial advertisement (`PARTIAL_CAPABILITY_ADVERTISEMENT`), MUST declare the closed storage operation vocabulary reconciled with accepted Platform Contract Proposal §5: the 15-operation baseline (`PutObject`, `GetObject`, `HeadObject`, `DeleteObject`, `DeleteObjects`, `ListObjectsV2`, `HeadBucket`, `CreateBucket`, `CreateMultipartUpload`, `UploadPart`, `CompleteMultipartUpload`, `AbortMultipartUpload`, `ListParts`, `PutBucketVersioning`, `GetBucketVersioning`) on non-immutable profiles, or the complete 19-operation inventory when Object Lock is included (adding `PutObjectRetention`, `GetObjectRetention`, `PutObjectLegalHold`, `GetObjectLegalHold`) in `supported_features` as a strictly closed set of enumerated baseline operations; any surplus operation (such as `DeleteBucket`) or undeclared operation is strictly forbidden and rejected. Payload inputs are strictly type-gated (`string`, `Buffer`, `Uint8Array` only; `null`/`undefined`/`Date`/`Uint16Array`/`Function` fail closed with `TypeError` / `MALFORMED_PAYLOAD_TYPE`). Object Lock PASS evidence is mandatory only when the target deployment profile mandates immutable storage (`immutable_storage_required: true`); when immutable storage is not mandated, Object Lock evidence is optional. When Object Lock evidence is present or mandated, it MUST strictly use the canonical Object Lock evidence URN (`urn:cybrik:evidence:storage:s3:conformance:v1:object-lock`) with verifiable passing status (`status: "PASS"`). `storage` slot capabilities can reference both `urn:cybrik:evidence:storage:s3:conformance:v1:s3-17-ops` (or general storage operations) and `urn:cybrik:evidence:storage:s3:conformance:v1:object-lock`, and exclusivity to `urn:cybrik:evidence:storage:s3:conformance:v1:object-lock` applies strictly to Object Lock retention conformance evidence. Object Lock alias detection strictly checks intent substrings `object-lock`, `object_lock`, and `objectlock` (prohibiting Object Lock aliases such as `urn:cybrik:evidence:storage:object-lock:v1` or `urn:cybrik:evidence:storage-object-lock:...`), while generic PASS storage evidence (e.g. `urn:cybrik:evidence:storage:s3:conformance:v1:custom-s3-ops`) is fully permitted, resolving to a `conformance_evidence[].test_identifier` entry bearing verified passing status (`status: "PASS"`). All evidence URNs MUST be lowercase canonical strings matching `^urn:cybrik:evidence:[a-z0-9][a-z0-9-_:]*[a-z0-9]$`; case-variant URNs (e.g. `...:Object-Lock`, `...:OBJECT-LOCK`) are strictly forbidden and rejected.

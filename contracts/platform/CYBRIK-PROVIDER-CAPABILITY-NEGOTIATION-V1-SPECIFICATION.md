...
| `/advertisement_response/advertised_capabilities` | `array` | MinItems 1, unique `slot_id` items | Array of capability descriptors. Each `slot_id` in `advertised_capabilities` MUST be unique (duplicate slot advertisements strictly prohibited). |
...
| `/advertisement_response/conformance_evidence[].status` | `string` | Enum `["PASS", "FAIL", "INCONCLUSIVE", "SKIPPED"]` | Test execution status; all valid conformance evidence records MUST carry `status: "PASS"`. Non-passing statuses are rejected. |

...
| `/advertisement_response/advertised_capabilities[].evidence_references` | `array` | MinItems 1, items pattern `^urn:cybrik:evidence:[a-zA-Z0-9_-]+(?::[a-zA-Z0-9_-]+)*$` | Evidence URN references backing the advertised capability. `storage` slot capabilities can reference both `urn:cybrik:evidence:storage:s3:conformance:v1:s3-17-ops` and `urn:cybrik:evidence:storage:s3:conformance:v1:object-lock`, where exclusivity to `urn:cybrik:evidence:storage:s3:conformance:v1:object-lock` applies strictly to Object Lock retention conformance evidence (prohibiting Object Lock aliases while permitting and expecting general storage operation PASS evidence). |
...
| `/advertisement_response/conformance_evidence[].test_identifier` | `string` | Pattern `^urn:cybrik:evidence:[a-zA-Z0-9_-]+(?::[a-zA-Z0-9_-]+)*$` | Unique test evidence URN identifier (for storage capabilities, supporting dual evidence `urn:cybrik:evidence:storage:s3:conformance:v1:s3-17-ops` and exclusive canonical Object Lock evidence `urn:cybrik:evidence:storage:s3:conformance:v1:object-lock`), requiring strict 1-to-1 bijection with `evidence_references` and verified passing status (`status: "PASS"`). |

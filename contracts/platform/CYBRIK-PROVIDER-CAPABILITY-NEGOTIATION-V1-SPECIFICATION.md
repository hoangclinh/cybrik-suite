...
Furthermore, each `slot_id` in `advertised_capabilities` MUST be unique across the advertisement (duplicate slot advertisements strictly prohibited). Any advertisement containing duplicate slot definitions MUST be rejected immediately (`TERMINAL_REJECTED`).
...
6. **Unconditional Target Profile Digest Binding**: EVERY capability advertisement (whether `FULL_PROFILE_CONFORMANCE_DECLARATION` or `PARTIAL_CAPABILITY_ADVERTISEMENT`) MUST bind a valid `target_profile_digest` matching the target profile SHA-256 digest (`target_profile_digest` matching `^[a-f0-9]{64}$`), preventing unanchored capability claims and profile downgrade attacks.
...
| `/target_profile_digest` | `string` | Pattern `^[a-f0-9]{64}$` | SHA-256 digest of the referenced deployment profile artifact. EVERY capability advertisement (full or partial) and negotiation handshake MUST bind a valid `target_profile_digest` matching the target profile SHA-256 digest. |
...
| `/advertisement_response/advertised_capabilities[].evidence_references` | `array` | MinItems 1, items pattern `^urn:cybrik:evidence:[a-zA-Z0-9_-]+(?::[a-zA-Z0-9_-]+)*$` | Evidence URN references backing the advertised capability. For storage Object Lock, canonical naming is `urn:cybrik:evidence:storage:s3:conformance:v1:object-lock` (or `urn:cybrik:evidence:storage:object-lock:v1` / `urn:cybrik:evidence:storage-object-lock:...`). |
...
| `/advertisement_response/conformance_evidence[].test_identifier` | `string` | Pattern `^urn:cybrik:evidence:[a-zA-Z0-9_-]+(?::[a-zA-Z0-9_-]+)*$` | Unique test evidence URN identifier (e.g. `urn:cybrik:evidence:storage:object-lock:v1` or `urn:cybrik:evidence:storage-object-lock:v1`), requiring strict 1-to-1 bijection with `evidence_references` and verified passing status (`status: "PASS"`). |
...
2. **Profile Digest Matching (`SR-NEG-02`)**: `target_profile_digest` (on root document, capability advertisements whether full or partial, and `agreed_capability_lease`) MUST exactly match the SHA-256 hash of the on-disk `contracts/examples/platform/<target_profile_id>.profile.json` document.

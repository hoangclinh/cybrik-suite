...
* `requested_optional_capabilities`: Desired optional features with explicit `capability_name`, `slot_id`, `required_for_optimal` (boolean indicating whether optimal operational posture requires this capability), and preferred fallback modes.
...
| `AGREED_LEASE_GRANTED` / `ACTIVE_OPTIMAL` | All mandatory slots valid; every requested capability with `required_for_optimal: true` resolved and granted fully without fallback (`disposition: "GRANTED_FULL"`, `fallback_applied: "NONE"`), with all resolved lease capabilities granted fully; missing capabilities with `required_for_optimal: false` (or omitted/false) do not prevent `ACTIVE_OPTIMAL`. Strict 1-to-1 status pair. | Full performance / full feature operation. |
| `DEGRADED_LEASE_GRANTED` / `ACTIVE_DEGRADED` | All mandatory slots valid; at least one capability degraded to fallback or at least one requested capability with `required_for_optimal: true` missing/unresolved. Strict 1-to-1 status pair. | Normal operation under documented fallback constraints. |
...
| `/negotiation_request/requested_optional_capabilities` | `array` | Unique composite keys `(capability_name, slot_id)` | Array of requested optional capability descriptors declaring `capability_name`, `slot_id`, `required_for_optimal`, and `preferred_fallback`. Both `requested_optional_capabilities` and `negotiated_optional_capabilities` arrays MUST NOT contain duplicate composite identity `(capability_name, slot_id)` entries (strict composite key set uniqueness per array). |
| `/negotiation_request/requested_optional_capabilities[].required_for_optimal` | `boolean` | Boolean flag | Indicates whether the optional capability is required to achieve optimal operational posture. If `true` and the capability is missing or degraded, lease status becomes `ACTIVE_DEGRADED`. If `false` (or omitted), a missing capability does not prevent `ACTIVE_OPTIMAL` status. |

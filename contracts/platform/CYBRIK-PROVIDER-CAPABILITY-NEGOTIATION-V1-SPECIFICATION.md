...
7. **Non-Degradability and Mandatory Lease Presence of Immutable Storage (`SR-NEG-07`)**: Whenever the target deployment profile mandates immutable storage (`profile.slots.storage.specification.immutable_storage_required === true` or `profile.immutable_storage_required === true`), storage Object Lock WORM retention is strictly mandatory and non-degradable: the exact capability `storage_object_lock` (with `slot_id: "storage"`; aliases, substrings, or alternative capability names strictly prohibited) MUST be present in `agreed_capability_lease.negotiated_optional_capabilities` AND MUST have `disposition: "GRANTED_FULL"` and `fallback_applied: "NONE"`. NO capability in `agreed_capability_lease.negotiated_optional_capabilities` or `agreed_capabilities` with `slot_id === 'storage'` or `capability_name === 'storage_object_lock'` may have `disposition: "GRANTED_DEGRADED"`, `status: "GRANTED_DEGRADED"`, `disposition: "REJECTED_UNSUPPORTED"`, `status: "REJECTED_UNSUPPORTED"`, or non-`"NONE"` fallback. If `storage_object_lock` is omitted or degraded, or if any alias (such as `storage_lock_alias`) is provided instead when `immutable_storage_required: true`, negotiation fails closed (`throw new Error("Semantic error: immutable storage profile requires storage_object_lock capability in lease with GRANTED_FULL disposition")` or `DEGRADATION_OF_IMMUTABLE_STORAGE_FORBIDDEN`).

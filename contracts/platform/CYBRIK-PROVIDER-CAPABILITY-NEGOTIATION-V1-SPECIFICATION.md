...
* `conformance_evidence`: Concrete test execution records (`test_identifier` matching URN pattern `^urn:cybrik:evidence:[a-zA-Z0-9_-]+(?::[a-zA-Z0-9_-]+)*$`, `verification_method`, `report_uri`). Each record verifies test execution with an exact 1-to-1 matching requirement against capability evidence references and confirmed passing status (`status: "PASS"`).

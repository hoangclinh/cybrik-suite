...
* **Own-Property Resolution on Multipart Upload Manifests & Part Descriptors**: All multipart upload manifest properties (including `manifest`, `parts`, `total_parts`, `total_size_bytes`) and part object properties (e.g., `part_number`, `etag`, `size_bytes`) MUST be evaluated strictly as own properties (`Object.prototype.hasOwnProperty.call(...)`). Prototype-inherited manifest structures, inherited parts arrays, or prototype-inherited totals/properties MUST be rejected and fail closed: an inherited parts array or inherited manifest wrapper triggers HTTP 400 `<Code>InvalidArgument</Code>` (`EMPTY_PARTS_LIST` / semantic validation error), while inherited `total_parts` or `total_size_bytes` triggers HTTP 400 `<Code>InvalidPart</Code>` (`TotalPartsMismatch` / `TotalSizeMismatch`).
...
| 2 | `InvalidDigest` | `400 Bad Request` | The `Content-MD5` or digest header value is malformed (e.g., non-base64 characters or decoded length $\neq 16$ bytes for MD5, malformed/uppercase/whitespace SHA-256 string, or unsupported `STREAMING-*` payload headers such as `STREAMING-AWS4-HMAC-SHA256-PAYLOAD` triggering `UNSUPPORTED_STREAMING_PAYLOAD_SHA256`), mandatory `x-amz-content-sha256` header is missing on `PutObject`, or headers/payload inputs fail own-property resolution (inherited headers or payload properties triggering `MALFORMED_HEADER_SYNTAX`). `STREAMING-*` SHA-256 header validation is executed unconditionally before any MD5 validation or payload digest comparisons, immediately returning HTTP 400 `InvalidDigest` (`UNSUPPORTED_STREAMING_PAYLOAD_SHA256`). Strictly mandated for malformed digest strings, unsupported streaming payload sentinels, missing `x-amz-content-sha256`, or prototype-inherited headers (providers MUST NOT return `InvalidArgument` or `BadDigest`). |
...
| 11 | `InvalidArgument` | `400 Bad Request` | An invalid argument or malformed XML body was supplied (e.g., part number outside 1–10,000 `INVALID_PART_NUMBER`, part count exceeding 10,000 `TOO_MANY_PARTS`, empty or prototype-inherited parts list `EMPTY_PARTS_LIST`, unknown retention mode, malformed XML structure). MUST NOT be returned in lieu of `BadDigest` or `InvalidDigest`. |

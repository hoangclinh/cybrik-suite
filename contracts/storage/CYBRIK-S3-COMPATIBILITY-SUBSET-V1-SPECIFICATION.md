...
* **`INV-S3-01` (Closed Operation Inventory — 15-Op Baseline / 19 Ops Profile-Aware Closure)**: The provider MUST implement the closed operation vocabulary reconciled with accepted Platform Contract Proposal §5: exactly all 15 baseline operations (8 CRUD/Bucket: `PutObject`, `GetObject`, `HeadObject`, `DeleteObject`, `DeleteObjects`, `ListObjectsV2`, `HeadBucket`, `CreateBucket`; 5 Multipart: `CreateMultipartUpload`, `UploadPart`, `CompleteMultipartUpload`, `AbortMultipartUpload`, `ListParts`; 2 Bucket Versioning: `PutBucketVersioning`, `GetBucketVersioning`) on non-immutable deployment profiles (`immutable_storage_required: false`), and exactly all 19 operations (including the 4 Object Lock operations: `PutObjectRetention`, `GetObjectRetention`, `PutObjectLegalHold`, `GetObjectLegalHold`) on immutable profiles (`immutable_storage_required: true`). Declarations or advertisements missing versioning operations (e.g., declaring only 17 operations omitting `PutBucketVersioning` and `GetBucketVersioning`) on immutable profiles are strictly rejected. Furthermore, `supported_features` arrays across storage capability descriptors MUST contain unique items (`uniqueItems: true`; duplicate items strictly prohibited and rejected). No unapproved subset omission or surplus operation is permitted.
...
* **Multipart Manifest Ordering & Cardinality Rules**:
  ...
  * Manifest wrappers, manifests, manifest parts, and stored parts MUST be plain objects (or null-prototype objects) whose fields are own plain data properties (`Object.prototype.hasOwnProperty`). Class instances, non-plain prototypes, getter/setter accessor properties, Proxy traps, malformed direct-array wrappers, or prototype-inherited structures MUST fail closed with HTTP 400 `InvalidPart` (`INVALID_MULTIPART_MANIFEST_STRUCTURE` / semantic validation error), including malformed direct-array wrappers or non-plain wrappers, malformed manifest structures, class instances, or prototype-inherited part descriptors / totals; while ONLY an explicitly valid, structurally sound empty array `parts: []` maps to HTTP 400 `InvalidArgument` (`EMPTY_PARTS_LIST`).
...
* **Failure Modes**:
  ...
  * `400 Bad Request` (`InvalidArgument`): Manifest XML is malformed, part count exceeds 10,000 (`TOO_MANY_PARTS`), part number is outside the valid range 1–10,000 (`INVALID_PART_NUMBER`), or manifest contains an explicitly valid empty parts array (`parts: []` -> `EMPTY_PARTS_LIST`).
...
### 5.20 URL Presigning Specification (`GetObject`, `PutObject`)

URL presigning enables authenticated platform components to delegate temporary read (`GetObject`) or write (`PutObject`) access to external consumers, unprivileged workers, or client-side agents without distributing long-lived cryptographic secrets or IAM credentials.

* **Supported Operations**:
  * `GetObject`: Temporary presigned download delegation.
  * `PutObject`: Temporary presigned single-object upload delegation.
* **HTTP Methods**: `GET` (for `GetObject`) | `PUT` (for `PutObject`)
* **Resource Path**: `/{bucket}/{key+}`
* **Authentication Scheme**: AWS Signature Version 4 Query Parameters (`AWS4-HMAC-SHA256`). Presigned URLs convey authentication context entirely within URI query parameters rather than HTTP request headers.
* **Mandatory SigV4 Query Parameters**:
  * `X-Amz-Algorithm`: Signature algorithm identifier, strictly `AWS4-HMAC-SHA256`.
  * `X-Amz-Credential`: Scoped access credential string formatted as `<access-key-id>/<date-stamp>/<region>/s3/aws4_request` (where `<date-stamp>` matches `YYYYMMDD` from `X-Amz-Date`).
  * `X-Amz-Date`: ISO 8601 basic UTC timestamp (`YYYYMMDD'T'HHMMSS'Z'`).
  * `X-Amz-Expires`: Expiration lifetime in seconds as an integer string ($1 \le \text{X-Amz-Expires} \le 604800$).
  * `X-Amz-SignedHeaders`: Semicolon-delimited list of lowercased header names included in the canonical request signature calculation (MUST include `host`; optional request headers such as `content-type` or `x-amz-meta-*` may be included).
  * `X-Amz-Signature`: 64-character lowercase hexadecimal HMAC-SHA256 signature calculated over the canonical request string to sign.
* **Optional Query Parameters**:
  * `X-Amz-Security-Token`: Temporary session token when authenticating via temporary or assumed-role credentials.
  * `versionId`: (For `GetObject`) Specific version ID to retrieve.
  * `response-content-disposition`, `response-content-type`: (For `GetObject`) Response header overrides.
* **Bounded Expiration Invariants**:
  * **Minimum Lifetime**: 1 second ($1\,\text{s}$).
  * **Maximum Lifetime**: 604,800 seconds ($7\,\text{days} = 604,800\,\text{s}$).
  * Non-integer, non-numeric, zero, negative ($< 1$), or excessive ($> 604800$) `X-Amz-Expires` values MUST fail closed with HTTP 400 `InvalidArgument` or HTTP 403 `AccessDenied`.
* **Canonical Request Construction for Presigned URLs**:
  * The canonical query string MUST include all query parameters present in the request (including `X-Amz-Algorithm`, `X-Amz-Credential`, `X-Amz-Date`, `X-Amz-Expires`, and `X-Amz-SignedHeaders`), sorted lexicographically by parameter name byte order, excluding `X-Amz-Signature`.
  * The payload hash (`HashedPayload`) in the canonical request for presigned URLs MUST be `'UNSIGNED-PAYLOAD'`.
* **Signature Verification & Expiry Evaluation**:
  * **Timestamp Expiration**: The storage provider MUST verify that the request arrival timestamp $\text{RequestTime}$ does not exceed the valid window:
    $$\text{X-Amz-Date} \le \text{RequestTime} \le \text{X-Amz-Date} + \text{X-Amz-Expires}$$
    If $\text{RequestTime} > \text{X-Amz-Date} + \text{X-Amz-Expires}$ (or if $\text{RequestTime}$ is prior to $\text{X-Amz-Date}$ allowing for standard clock skew $\le 15\,\text{minutes}$), the request MUST fail closed with HTTP 403 `AccessDenied` (`Request has expired`).
  * **Signature Matching**: The computed signature over the canonical request and signing key must match `X-Amz-Signature` exactly via constant-time comparison. Mismatch MUST fail closed with HTTP 403 `AccessDenied` (`SignatureDoesNotMatch`).
  * **Signed Headers Integrity**: All headers listed in `X-Amz-SignedHeaders` must be present and match their signing values on the incoming HTTP request.
* **Success Responses**:
  * For `GetObject`: Identical to standard `GetObject` response (HTTP 200 OK or HTTP 206 Partial Content with payload body and object metadata).
  * For `PutObject`: Identical to standard `PutObject` response (HTTP 200 OK with `ETag` and version header).
* **Failure Modes**:
  * `400 Bad Request` (`InvalidArgument`): Missing required query parameters, malformed parameter syntax, or out-of-bounds `X-Amz-Expires` ($< 1$ or $> 604800$).
  * `403 Forbidden` (`AccessDenied`): Presigned URL has expired, signature does not match, credential is invalid or revoked, or signed header is missing/altered.
  * `404 Not Found` (`NoSuchBucket` / `NoSuchKey`): Target bucket or key does not exist.
...
### 8.2 Payload Digest Validation (`Content-MD5` and `x-amz-content-sha256`), Raw Octet Hashing, Payload Type Gating, Accessor Safety & Part Integrity Verification
* **Payload Type Gating & Permitted Types**: Payload inputs across storage operations and digest calculations MUST be strictly type-gated. Only `string` (evaluated strictly as raw UTF-8 octets without speculative decoding), `Buffer`, and `Uint8Array` are permitted payload representations. Any unsupported or invalid types—specifically including explicit `null`, `undefined` (when a payload argument is passed or expected), `Date`, `Uint16Array`, `Function`, plain object, number, boolean, array, or other structured non-byte types—MUST fail closed:
  ...
  - In protocol dispatch and request validation pipelines (`dispatchS3PutObject`, `dispatchS3Error`), explicit `null` and `undefined` payloads (along with all other non-permitted types) uniformly return HTTP 400 `<Code>InvalidDigest</Code>` and reason `MALFORMED_PAYLOAD_TYPE`.
* **Accessor Properties Prohibition & Plain Data Properties Invariant**: Accessor properties (`get`/`set` getters and setters, whether defined via `Object.defineProperty`, `Object.defineProperties`, or class declarations) are strictly prohibited on contract input objects, request options, headers dictionaries, manifest wrappers, and part descriptors. Callers MUST pass plain data properties (own enumerable data properties directly assigned on plain objects with `Object.prototype` or `null` prototype). Any object containing getter/setter accessors or prototype-inherited accessor properties MUST fail closed (returning HTTP 400 `InvalidDigest` with `MALFORMED_HEADER_SYNTAX` / `MALFORMED_PAYLOAD_TYPE` or HTTP 400 `InvalidArgument` / `InvalidPart` in storage pipelines, and failing schema / semantic conformance).
...
* **`PutObject` Payload Digest Verification**: On `PutObject`, the server MUST verify the received payload against the 64-character lowercase hexadecimal SHA-256 digest provided in `x-amz-content-sha256` (or profile-authorized `'UNSIGNED-PAYLOAD'`) and the Base64 MD5 digest provided in `Content-MD5` (when provided).
  ...
  * Syntactically invalid digest header values (e.g., missing mandatory header on `PutObject`, non-hex characters, uppercase characters, or length $\neq 64$ in SHA-256 headers, unsupported `STREAMING-*` payload headers, or malformed Base64 in `Content-MD5`), invalid payload types (explicit `null`, `undefined`, `Date`, `Uint16Array`, `Function`, plain objects, numbers, booleans, arrays, or other structured non-byte types triggering `MALFORMED_PAYLOAD_TYPE` / `TypeError`), or accessor properties / prototype-inherited headers MUST be rejected strictly with HTTP 400 `<Code>InvalidDigest</Code>`. Storage providers MUST NOT return `InvalidArgument` for malformed digest headers or invalid payload types.
...
### 8.4 Multipart Manifest Ordering, Cardinality, Part Integrity, Total Invariants & Size Thresholds (`INV-S3-10`)
...
* **Plain Object Prototype, Own Plain Data Property Resolution & Proxy Safety on Multipart Upload Manifests & Part Descriptors**: Manifest wrappers, manifests, manifest parts, and stored parts MUST be plain objects (or null-prototype objects) whose fields are own plain data properties (`Object.prototype.hasOwnProperty`). Class instances, accessor properties (`get`/`set`), non-plain prototypes, Proxy traps, malformed direct-array wrappers or manifests, or prototype-inherited properties MUST fail closed with HTTP 400 `<Code>InvalidPart</Code>` (`INVALID_MULTIPART_MANIFEST_STRUCTURE` / semantic validation error):
  * Manifest wrappers and manifests MUST be plain objects (or null-prototype objects) with plain data properties. Passing malformed direct-array wrappers, non-plain prototype wrappers, class instances, getter/setter accessor properties, Proxy get traps, non-array `parts`, or prototype-inherited manifest structures (such as inherited `manifest` wrappers or inherited `parts` arrays) MUST fail closed with HTTP 400 `<Code>InvalidPart</Code>` (`INVALID_MULTIPART_MANIFEST_STRUCTURE` / semantic validation error).
  * Manifest parts and stored parts MUST be plain objects (or null-prototype objects) whose fields (`part_number`, `etag`, `size_bytes`, `sha256`) are resolved strictly as own plain data properties (`Object.prototype.hasOwnProperty.call(...)`). Any class instances, accessor properties (`get`/`set`), non-plain prototypes, Proxy get traps, or prototype-inherited part fields (`part_number`, `etag`, `size_bytes`) on manifest parts or stored parts MUST fail closed with HTTP 400 `<Code>InvalidPart</Code>` (`INVALID_MULTIPART_MANIFEST_STRUCTURE` / `InvalidPartNumber` / `MissingManifestPartETag` / `MissingStoredPartETag` / `InvalidPartSize`).
  * Declared totals (`total_parts`, `total_size_bytes`) MUST be evaluated strictly as own plain data properties; accessor properties or prototype-inherited `total_parts` or `total_size_bytes` MUST fail closed with HTTP 400 `<Code>InvalidPart</Code>` (`TotalPartsMismatch` / `TotalSizeMismatch` / `INVALID_MULTIPART_MANIFEST_STRUCTURE`).
  * ONLY an explicit, structurally valid empty array `parts: []` returns HTTP 400 `<Code>InvalidArgument</Code>` (`EMPTY_PARTS_LIST`).
* **Cardinality & Part Bounds**:
  ...
  * **Empty Manifest**: Completing a multipart upload with an explicitly valid empty parts array (`parts: []` or 0 parts) within a valid manifest structure returns HTTP 400 `<Code>InvalidArgument</Code>` (`EMPTY_PARTS_LIST`), not `InvalidPart`. Passing a direct array wrapper (such as `[]`) or non-plain wrapper is a malformed wrapper structure and MUST fail closed with HTTP 400 `<Code>InvalidPart</Code>` (`INVALID_MULTIPART_MANIFEST_STRUCTURE`). Duplicate part numbers violate strictly ascending order and MUST be rejected with HTTP 400 `InvalidPartOrder`.
...
| 2 | `InvalidDigest` | `400 Bad Request` | The `Content-MD5` or digest header value is malformed (e.g., non-base64 characters or decoded length $\neq 16$ bytes for MD5, malformed/uppercase/whitespace SHA-256 string, or unsupported `STREAMING-*` payload headers such as `STREAMING-AWS4-HMAC-SHA256-PAYLOAD` triggering `UNSUPPORTED_STREAMING_PAYLOAD_SHA256`), mandatory `x-amz-content-sha256` header is missing on `PutObject`, payload inputs fail type gating (explicit `null`, `undefined`, `Date`, `Uint16Array`, `Function`, plain objects, numbers, booleans, arrays, or other structured non-byte types failing raw octet buffer or UTF-8 string requirement, triggering `TypeError` in helper routines or `MALFORMED_PAYLOAD_TYPE` in dispatchers), or headers/payload inputs fail own-property / plain data property resolution (accessor properties `get`/`set` or inherited headers/payload properties triggering `MALFORMED_HEADER_SYNTAX` / `MALFORMED_PAYLOAD_TYPE`). `STREAMING-*` SHA-256 header validation is executed unconditionally before any MD5 validation or payload digest comparisons, immediately returning HTTP 400 `InvalidDigest` (`UNSUPPORTED_STREAMING_PAYLOAD_SHA256`). Strictly mandated for malformed digest strings, unsupported streaming payload sentinels, missing `x-amz-content-sha256`, invalid payload types (`MALFORMED_PAYLOAD_TYPE`), or prototype-inherited/accessor headers (providers MUST NOT return `InvalidArgument` or `BadDigest`). |

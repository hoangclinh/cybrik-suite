...
* **Failure Modes**:
  ...
  * `400 Bad Request` (`InvalidDigest`): `x-amz-content-sha256` header is missing, malformed (not matching `^[a-f0-9]{64}$` or `'UNSIGNED-PAYLOAD'`), or `Content-MD5` header is malformed (not valid 128-bit Base64).
...
* **Multipart Manifest Ordering & Cardinality Rules**:
  ...
  * Declared `total_parts` and `total_size_bytes` (if provided) MUST exactly equal the actual count of parts and the sum of stored part sizes; any mismatch returns HTTP 400 `InvalidPart`.
...
* **Success Response**:
  ...
  * **Response Body**: XML document returning `CompleteMultipartUploadResult`:
    ...
    ```
* **Failure Modes**:
  * `400 Bad Request` (`InvalidPart`): Declared part ETag does not match stored part ETag, referenced part was not uploaded, missing / non-integer stored part size (`sp.size`, `sp.size_bytes`, `sp.Size` -> `InvalidPartSize`), or declared `total_parts` / `total_size_bytes` mismatch against actual parts count or sum of stored parts byte sizes.
  ...
  * `400 Bad Request` (`InvalidArgument`): Manifest XML is malformed, part count exceeds 10,000 (`TOO_MANY_PARTS`), part number is outside the valid range 1–10,000 (`INVALID_PART_NUMBER`), or manifest contains zero parts (`EMPTY_PARTS_LIST`).
...
4. **Object Key Normalization & Dot-Segment Rejection**: All `s3://` URIs (`s3://{bucket}/{key}`) and path-style URLs (`http(s)://{endpoint}/{bucket}/{key}`) MUST conform to normalized object keys without dot-segments (`..`, `/.`), repeated slashes (`//`), or trailing slashes (`/`). Specifically:
   ...
   * **Normative Key Pattern**: Object keys MUST strictly match the normative regex pattern `^(?!\\/)(?!^\\.\\/)(?!.*\\.\\.)(?!.*(?:\\/\\.|\\/\\/|\\/$))[a-zA-Z0-9._/-]+$`.
...
* **`PutObject` Payload Digest Verification**: On `PutObject`, the server MUST verify the received payload against the 64-character lowercase hexadecimal SHA-256 digest provided in `x-amz-content-sha256` (or profile-authorized `'UNSIGNED-PAYLOAD'`) and the Base64 MD5 digest provided in `Content-MD5` (when provided).
  ...
  * `STREAMING-AWS4-HMAC-SHA256-PAYLOAD` is not part of this closed subset and must be rejected with HTTP 400 `<Code>InvalidDigest</Code>` (or `<Code>BadDigest</Code>` upon payload verification failure).
...
* **Declared Total Invariants (`total_parts` and `total_size_bytes`)**: In `CompleteMultipartUpload`, declared `total_parts` and `total_size_bytes` (if provided in the manifest or upload completion request):
  * **`total_parts` Invariant**: The declared `total_parts` value MUST exactly equal the actual count of parts in the completion manifest ($|\text{manifest.parts}|$). Any mismatch between declared `total_parts` and actual parts count MUST be rejected with HTTP 400 `<Code>InvalidPart</Code>`.
  * **`total_size_bytes` Invariant**: The declared `total_size_bytes` value MUST exactly equal the aggregate sum of stored parts byte sizes ($\sum_{i=1}^n \text{storedPart}_i.\text{size\_bytes}$). Any mismatch between declared `total_size_bytes` and the computed sum of stored parts byte sizes MUST be rejected with HTTP 400 `<Code>InvalidPart</Code>`.
...
| 2 | `InvalidDigest` | `400 Bad Request` | The `Content-MD5` or digest header value is malformed (e.g., non-base64 characters or decoded length $\neq 16$ bytes for MD5, or malformed SHA-256 string) or mandatory `x-amz-content-sha256` header is missing on `PutObject`. Strictly mandated for malformed digest strings or missing `x-amz-content-sha256` (providers MUST NOT return `InvalidArgument`). |
...
| 11 | `InvalidArgument` | `400 Bad Request` | An invalid argument or malformed XML body was supplied (e.g., part number outside 1–10,000 `INVALID_PART_NUMBER`, part count exceeding 10,000 `TOO_MANY_PARTS`, empty parts list `EMPTY_PARTS_LIST`, unknown retention mode, malformed XML structure). MUST NOT be returned in lieu of `BadDigest` or `InvalidDigest`. |
| 12 | `InvalidPart` | `400 Bad Request` | One or more declared parts in `CompleteMultipartUpload` were not found, the provided part ETag does not match the stored part ETag, a completed part lacks a mandatory valid integer stored-part size (`sp.size`, `sp.size_bytes`, `sp.Size` -> `InvalidPartSize`), or declared `total_parts` / `total_size_bytes` (if provided) does not match actual parts count or sum of stored parts byte sizes. |

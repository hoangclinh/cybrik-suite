...
* **Multipart Manifest Ordering & Cardinality Rules**:
  ...
  * Manifest wrappers, manifests, manifest parts, and stored parts MUST be plain objects (or null-prototype objects) whose fields are own plain data properties (`Object.prototype.hasOwnProperty`). Class instances, non-plain prototypes, getter/setter accessor properties, Proxy traps, or prototype-inherited structures MUST fail closed with HTTP 400 `InvalidPart` (`INVALID_MULTIPART_MANIFEST_STRUCTURE` / semantic validation error), including malformed manifest structures, non-plain prototype wrappers, class instances, or prototype-inherited part descriptors / totals; while an explicitly valid empty array `parts: []` maps to HTTP 400 `InvalidArgument` (`EMPTY_PARTS_LIST`).

...
* **Success Response**:
  ...
  * **Response Body**: XML document returning `CompleteMultipartUploadResult`:
    ...
    ```
* **Failure Modes**:
  * `400 Bad Request` (`InvalidPart`): Declared part ETag does not match stored part ETag, referenced part was not uploaded, or missing / non-integer stored part size (`sp.size`, `sp.size_bytes`, `sp.Size` -> `InvalidPartSize`).
...
| 12 | `InvalidPart` | `400 Bad Request` | One or more declared parts in `CompleteMultipartUpload` were not found, the provided part ETag does not match the stored part ETag, or a completed part lacks a mandatory valid integer stored-part size (`sp.size`, `sp.size_bytes`, `sp.Size` -> `InvalidPartSize`). |

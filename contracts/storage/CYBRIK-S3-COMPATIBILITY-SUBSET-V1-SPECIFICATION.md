...
* **`INV-S3-05` (Strict End-to-End Digest Verification & Strict Error Dispatch)**: The provider MUST compute and validate payload digests against `Content-MD5` and `x-amz-content-sha256` headers. The `x-amz-content-sha256` header accepts only a 64-hex lowercase SHA-256 digest or profile-authorized `UNSIGNED-PAYLOAD`. `STREAMING-AWS4-HMAC-SHA256-PAYLOAD` is not part of this closed subset and must be rejected with `InvalidDigest` / `BadDigest`. On `PutObject`, the provider MUST compute the payload SHA-256 digest and verify it against `x-amz-content-sha256` (when provided and not `UNSIGNED-PAYLOAD`) as well as verifying `Content-MD5`, failing closed strictly with `BadDigest` (HTTP 400) on payload digest mismatch and `InvalidDigest` (HTTP 400) on malformed digest headers. Returning `InvalidArgument` or `AccessDenied` in lieu of `BadDigest`/`InvalidDigest` is strictly forbidden. On `CompleteMultipartUpload`, every referenced part in the completion manifest MUST exist and its declared ETag MUST match the stored part ETag recorded during `UploadPart` via exact string matching (without quote stripping or normalization); any missing part or stored-ETag mismatch MUST be rejected with HTTP 400 `InvalidPart`.
...
* **Multipart Manifest Ordering & Cardinality Rules**:
  * The manifest MUST contain at least 1 and at most 10,000 `<Part>` elements.
  ...
  * Every part except the final part in the assembled object MUST be at least 5 MiB (5,242,880 bytes).
* **Success Response**:
  * **HTTP Status**: `200 OK`
  * **Response Headers**: `Content-Type: application/xml`
  * **Response Body**: XML document returning `CompleteMultipartUploadResult`:
    ...
    ```
* **Failure Modes**:
  * `400 Bad Request` (`InvalidPart`): Declared part ETag does not match stored part ETag or referenced part was not uploaded.
  ...
  * `400 Bad Request` (`InvalidArgument`): Manifest XML is malformed, part number is outside the valid range (1–10,000), or manifest contains zero parts.
...
### 8.2 Payload Digest Validation (`Content-MD5` and `x-amz-content-sha256`) & Part Integrity Verification
* **`PutObject` Payload Digest Verification**: On `PutObject`, the server MUST verify the received payload against the 64-character lowercase hexadecimal SHA-256 digest provided in `x-amz-content-sha256` (or profile-authorized `UNSIGNED-PAYLOAD`) and the Base64 MD5 digest provided in `Content-MD5`.
  * The `x-amz-content-sha256` header accepts only a 64-hex lowercase SHA-256 digest or profile-authorized `UNSIGNED-PAYLOAD`. `STREAMING-AWS4-HMAC-SHA256-PAYLOAD` is not part of this closed subset and must be rejected with HTTP 400 `<Code>InvalidDigest</Code>` (or `<Code>BadDigest</Code>` upon payload verification failure).
  * Syntactically invalid digest header values (e.g., non-hex characters, uppercase characters, or length $\neq 64$ in SHA-256 headers, unsupported streaming payload auth, or malformed Base64 in `Content-MD5`) MUST be rejected strictly with HTTP 400 `<Code>InvalidDigest</Code>`. Storage providers MUST NOT return `InvalidArgument` for malformed digest headers.
...
### 8.4 Multipart Manifest Ordering, Cardinality, Part Integrity & Size Thresholds (`INV-S3-09`)
...
* **Cardinality & Part Uniqueness**: Completion manifests MUST contain between 1 and 10,000 parts. Completing a multipart upload with an empty parts list (`parts: []` or 0 parts) returns HTTP 400 `<Code>InvalidArgument</Code>` (`EMPTY_PARTS_LIST`), not `InvalidPart`. Duplicate part numbers violate strictly ascending order and MUST be rejected with HTTP 400 `InvalidPartOrder`.
...
* **Part & Object Size Thresholds**:
  * **Minimum Non-Final Part Size**: 5 MiB (5,242,880 bytes). All parts except the final part in the assembled object MUST be at least 5 MiB in size. Any non-final part smaller than 5 MiB MUST be rejected with HTTP 400 `<Code>EntityTooSmall</Code>`.
  ...
  * **Maximum Part Size**: 5 GiB (5,368,709,120 bytes). Any individual part exceeding 5 GiB MUST be rejected with HTTP 400 `<Code>EntityTooLarge</Code>`.
  * **Maximum Object Size**: 5 TiB (5,497,558,138,880 bytes). The total assembled object size across all completed parts (up to 10,000 parts $\times$ 5 GiB) MUST NOT exceed 5 TiB. Assembled objects exceeding 5 TiB MUST be rejected with HTTP 400 `<Code>EntityTooLarge</Code>`.
